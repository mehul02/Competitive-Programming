...
1207 C - Gas Pipeline.cpp GNU C++14 Accepted
///==================================================///
	///
	///  Ir0nic_  ///
    ///  MEHUL BHUTALIA  ///
...
ll rec(ll i,ll pos)
{
    ...
    if(pos==0)
    {
        if(s[i]=='1')
        return inf;
        return dp[i][pos]= min(2*a+b+rec(i+1,1),a+b+rec(i+1,0));
    }
    if(s[i]=='1')
        return dp[i][pos]= a+2*b+rec(i+1,1);
    return dp[i][pos]= min(2*a+2*b+rec(i+1,0),a+2*b+rec(i+1,1));
